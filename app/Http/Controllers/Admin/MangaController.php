...
class MangaController extends Controller
{
        ...
        if ($request->hasfile('chapters')) {
            ...
            foreach ($request->file('chapters') as $key => $file) {
                ...
                if ($file->getClientOriginalExtension() === 'zip') {
                    ...
                    $chapter_remove_dot = strtok($file_name, '.');
    ...
    public function uploadChapterToAWS($request, $id)
    {
        if ($request->hasfile('chapters')) {
            ...
            foreach ($request->file('chapters') as $key => $file) {
                $file_name = $file->getClientOriginalName();
                $chapter_name = strtok($file_name, '_');
                $chapter_remove_dot = strtok($file_name, '.');
                $chapter_number = ltrim(strstr($chapter_remove_dot, '_'), '_');;
                if (!strpos($chapter_remove_dot, '_')) {
                    $chapter_name = null;
                    $chapter_number = $chapter_remove_dot;
                }
                $zip = new ZipArchive();
                if ($zip->open($file) === TRUE) {
                    $dir_file = storage_path() . '/app/public/manga/' . Str::slug($manga->mangas_name, '_') . '/' . $chapter_number;
